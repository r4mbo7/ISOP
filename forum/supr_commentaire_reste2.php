<?php
	...
	$test = $bdd->query("SELECT auteur, id_auteur FROM commentaires_reste WHERE id = $id ");	 //recupération de l'auteur et son id
	$x = $test->fetch();
	...
	if ( $modo['adm'] == '2' OR $modo['adm'] == '3' OR $prenom == $id_auteur AND $_SESSION['commentaire'] == 1)
	{
		...
		if ($auteur != "Modérateur" AND $auteur != "Administrateur")
		{
			...
			$bdd->exec("DELETE FROM commentaires_reste WHERE id = '$id'");	//supression du comment
			echo "<script type='text/javascript'>history.go(-1);</script>";
		}
		elseif ($auteur == "Modérateur")
		{
			$bdd->exec("DELETE FROM commentaires_reste WHERE id = '$id'");
			echo "<script type='text/javascript'>history.go(-1);</script>";
		}
