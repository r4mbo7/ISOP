...
session_start();
	...
	if ( $modo['adm'] == '2' OR $modo['adm'] == '3')
	{
		...
		$id = $_GET['id'];
		$req=$bdd->query("SELECT image FROM billet_reste WHERE id = '$id'");
		$img=$req->fetch();
		if(!empty($img['image']))
		{
		$image=$img['image'];
		...
		$bdd->exec("DELETE FROM billet_reste WHERE id = '$id'");
		$bdd->exec("DELETE FROM commentaires_reste WHERE id_billet = '$id'");
